...
- **Nom du module** : [nom]
- **Objectif** : [quel calcul géométrique ce module réalise-t-il, en une ou deux phrases]
- **Texte descriptif public** : [texte affiché aux utilisateurs — but du module et ses limites (ce qu'il ne calcule pas / hypothèses simplificatrices), destiné à la page d'accueil du portail et à l'en-tête du module. Ce texte passe par le système de traduction (voir §6).]
- **Entrées utilisateur** : [quelles données l'utilisateur doit fournir — dimensions, angles, choix d'éléments types, etc.]
- **Résultats produits** : [quel calcul/résultat, sous quelle forme — valeur unique, tableau, dessin, ou combinaison]
- **Besoin d'une librairie d'éléments types personnalisable ?** [Oui/Non — si oui, préciser la nature des éléments, ex. segments de voie type, véhicules type, avec leurs attributs]
...
- **Calculs propres à ce module** : [décrire la ou les formules/algorithmes spécifiques]
- **Structure de données propre** : [ex. structure d'un "segment type" ou d'un "véhicule type" avec ses attributs ; structure d'un "projet" pour ce module]
- **Règles de validation des entrées** : [ex. valeurs minimales/maximales, cohérence entre paramètres]
- **Cas particuliers à gérer** : [le cas échéant]
