...
- **Nouvel export SVG** (`packages/commun`) : bouton d'export ajouté à `ExportButtons`
  partout où un module fournit `getSvgElement` (déjà le cas pour le PNG) — bénéficie donc
  immédiatement à tous les modules à dessin (`module-arc`, `module-empriselaterale`).
  `exportSvgToSvgFile`/`svgToSvgBlob` (`export/svg.ts`) clonent le SVG affiché et posent des
  attributs `width`/`height` en **mm réels** (déduits du `viewBox`, déjà en mm de dessin —
  même lecture que `getSvgMmSize`, réutilisé depuis `export/png.ts`), en plus du `viewBox`
  existant : un logiciel CAD (DraftSight, LibreCAD, Illustrator/Inkscape...) importe alors
  le dessin calibré à l'échelle 1:1 sans réglage manuel, contrairement à un PNG rastérisé
  qui n'embarque aucune métadonnée d'échelle fiable. Le `style` d'affichage écran
  (`max-width`/`height:auto` du layout responsive) est retiré du clone exporté, sans rapport
  avec le fichier livré.
